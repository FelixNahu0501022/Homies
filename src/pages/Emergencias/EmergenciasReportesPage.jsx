// src/pages/Emergencias/EmergenciasReportesPage.jsx - SIMPLIFIED
import {
...
const TIPOS_REPORTE = [
  { value: "listado_completo", label: "Emergencias - Listado Completo" },
];
...
export default function EmergenciasReportesPage() {
  ...
  const generarReporte = async () => {
    ...
    try {
      ...
      const emergencias = response.data?.data || response.data || [];

      const resultado = emergencias.map(e => ({
        ID: e.idemergencia,
        Fecha: e.fecha || "—",
        Tipo: e.tipo || "—",
  ...
  const exportarPDF = () => {
    ...
    exportTablePdf({ title: `Reporte: ${reporteSeleccionado?.label || tipoReporte}`, columns, rows: datos, filename: `emergencias_${tipoReporte}.pdf` });
  };
  ...
  return (
    <LayoutDashboard>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} flexWrap="wrap" gap={1}>
        <Box>
          <Typography variant="h5" fontWeight={700}>Reportes de Emergencias</Typography>
          <Typography variant="body2" color="text.secondary">Genera reportes con filtros personalizados</Typography>
        </Box>
        <Button variant="outlined" startIcon={<ArrowBack />} onClick={() => navigate("/emergencias")}>Volver</Button>
      </Box>
